...
def bootstrap_services(route_config_cls):
    ...
    with _bootstrap_lock:
        ...
        _log("bootstrap lock acquired")

        if BootstrapState.started:
            _log("bootstrap already started")
            return
    ...
    def _run():
        ...
        try:
            ...
            _log("Importing fuel module (numpy/scipy — this may take 1-2 min)...")
            t1 = time.monotonic()
    ...
    _log("starting bootstrap thread")

    thread.start()

    _log("bootstrap thread launched")
